...
public:
	...
	Dataset( _int argc, char* argv[] )
	{
		...
		if( dataset != "" )
		{
			...
			_bool* split = read_vec<_bool>( data_dir + "split." + to_string(split_no) + ".txt" );
		...
		else
		{
			...
			if( trn_ft_file != "" )
				trn_X_Xf = new SMatF( trn_ft_file, num_Xf );

			if( trn_lbl_file != "" )
				trn_X_Y = new SMatF( trn_lbl_file, num_Y );

			if( tst_ft_file != "" )
				tst_X_Xf = new SMatF( tst_ft_file, num_Xf );

			if( tst_lbl_file != "" )
				tst_X_Y = new SMatF( tst_lbl_file, num_Y );
		}
		...
		if( trn_X_Y != NULL )
		{
			...
			_int num_X = trn_X_Y->nc;
			_int num_Y = trn_X_Y->nr;
			_float C = (log((_double)num_X)-1)*pow(B+1,A);

			_int* freqs = new _int[ num_Y ]();
	...
	Dataset( string dataset, _int split_no=0 )
	{
		trn_X_Xf = NULL;
		trn_X_Y = NULL;
		tst_X_Xf = NULL;
